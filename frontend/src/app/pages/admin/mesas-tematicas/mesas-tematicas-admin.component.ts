    ...
    <div class="panel-page">
      ...
      <section class="alta-seccion">
        <div class="alta-seccion-head">
          <h2 class="alta-seccion-titulo">2. Trabajos orales</h2>
        ...
        @if (trabajosAbiertos) {
              ...
              @for (t of trabajosVisibles; track t.id) {
                <label class="check-row">
                  ...
                  <span>
                    {{ t.titulo }}
                    <span class="muted"
                      >({{ t.ejeTematico }} — {{ etiquetaModalidad(t.modalidad) }})</span
                    >
                  </span>
                </label>
              }
          ...
          <p class="muted small">
            Lista oculta.
            @if (seleccionados.size) {
              Tenés {{ seleccionados.size }} trabajo(s) seleccionado(s).
            }
          </p>
        }
      </section>
...
export class MesasTematicasAdminComponent implements OnInit {
  ...
  private intentarAplicarDefaults(): void {
    ...
    // aulas puede estar vacío todavía; igual aplicamos fecha/franja
    const defs = aplicarDefaultsAlta('MESA_TEMATICA', {
    ...
    if (defs.aulaId != null || defs.franjaId != null) {
      this.hintDefaults = 'Se precargó aula/horario de la última mesa temática que creaste.';
    }
